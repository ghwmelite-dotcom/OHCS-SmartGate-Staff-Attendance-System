...
// the clock page lists those entries and dismiss clears ONLY the failed ones.
// jsdom has no IndexedDB, so these tests run against a minimal in-memory fake
// that implements exactly the surface offlineQueue uses.
    ...
    return {
      ...
      transaction(storeName: string, _mode: string) {
        ...
        const tx: {
          ...
          onerror: (() => void) | null;
          objectStore: () => Record<string, (arg?: unknown) => unknown>;
...
async function seedRecord(rec: Record<string, unknown>): Promise<void> {
  const db = await new Promise<{ transaction: (s: string, m: string) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!d.objectStoreNames.contains('clock-queue')) d.createObjectStore('clock-queue');
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.objectStore('clock-queue').put(rec);
...
function failedRecord(id: string): Record<string, unknown> {
  return {
    ...
    attempts: 1,
    status: 'failed',
    failReason: 'Clock session expired',
    failedAt: Date.now(),
  };
}
...
describe('offlineQueue failed-entry handling', () => {
  ...
  it('lists no failed entries when the queue only holds pending/legacy records', async () => {
    ...
    expect(await listFailed('clock-queue')).toEqual([]);
    expect(await queueCount('clock-queue')).toBe(2);
  });

  it('retains failed entries and returns them with their reason', async () => {
    await seedRecord(failedRecord('x'));
    const failed = await listFailed('clock-queue');
    expect(failed).toHaveLength(1);
    expect(failed[0].id).toBe('x');
    expect(failed[0].failReason).toBe('Clock session expired');
    expect(failed[0].failedAt).toBeTypeOf('number');
    // Retained — still in the store.
    expect(await queueCount('clock-queue')).toBe(1);
  });
